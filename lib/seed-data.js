// lib/seed-data.js — the Hub's canonical statutory dataset, researched 2026-08-22 through
// 2026-08-23, in the payload shape lib/rates.js validates (matching what Shanti Ops' insert*/patch
// functions require — see PAYLOAD_VALIDATORS). This is the single source of truth for both the
// manual CLI (scripts/seed-rates.mjs) and the scheduled daily refresh (lib/refresh.js) — never
// duplicate this array, both callers import getSeedRows() from here.
//
// There is no live external API for any of this data (confirmed repeatedly this session — GST
// Accelerator's webhooks aren't live, Sandbox has no rate-lookup endpoint, no government API
// exists). This file changes only when a human edits it after researching a real law change (a
// Budget, a GST Council meeting, a new notification) — see the Hub's own README.
//
// source_ref suffixes:
...
export const SEED_SUBMITTED_BY = 'lib/seed-data.js (web research, 2026-08-22 to 2026-08-23)';
...
// Fixed placeholder, NOT `new Date()` — a handful of rows have no confirmed effective_from yet.
// Using "today" would make effective_from drift on every call (tomorrow's "today" > yesterday's
// stored value), which diffAndDraft would see as a genuine new version and draft again and again.
// Pin it to when this data was researched.
const UNKNOWN_EFFECTIVE_DATE = '2026-08-22';

// Returns a fresh copy each call — diffAndDraft mutates nothing it's handed, but callers (tests,
// the refresh job) shouldn't share one mutable array across runs either.
export function getSeedRows() {
  return ROWS.map((r) => ({
    ...r,
    effective_from: r.effective_from || UNKNOWN_EFFECTIVE_DATE,
    submitted_by: SEED_SUBMITTED_BY,
  }));
}
