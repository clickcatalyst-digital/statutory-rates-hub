...
async function migrate(client) {
  ...
  // retracted_* — for an APPROVED row that turns out to have been wrong from inception (a
  // data-entry error, not a later legal change). Retraction never touches category/payload/
  // effective_from/effective_to/source_ref — those stay exactly as originally approved, forever,
  // so the audit trail shows what was believed and when it was corrected, not a rewritten history.
  // A retracted row is excluded from listSince (never reaches a tenant) and from diffAndDraft's
  // conflict/version detection (a corrected replacement can reuse the same effective_from, since
  // the retracted row no longer counts as an "existing version" to conflict with).
  const { rows: existingCols } = await client.execute(`PRAGMA table_info(rate_changes)`);
  const colNames = new Set(existingCols.map((c) => c.name));
  if (!colNames.has('retracted_at')) await client.execute(`ALTER TABLE rate_changes ADD COLUMN retracted_at DATETIME`);
  if (!colNames.has('retracted_by')) await client.execute(`ALTER TABLE rate_changes ADD COLUMN retracted_by TEXT`);
  if (!colNames.has('retraction_reason')) await client.execute(`ALTER TABLE rate_changes ADD COLUMN retraction_reason TEXT`);

  // One row per daily-refresh-job run (lib/refresh.js) — the heartbeat record. status is
  // 'running' while in flight (so a crashed run is visible, not silently missing), then
  // 'success' or 'failed'. A monitoring check just needs "when did status last become success".
  await client.execute(`CREATE TABLE IF NOT EXISTS refresh_runs (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    unchanged INTEGER,
    rejected INTEGER,
    superseded_drafts_removed INTEGER,
    error_message TEXT
  )`);
}
