...
// zero drafts. Not handled here (deliberately): repeals (need effective_to on a live approved row,
// the one sanctioned exception to "never mutate approved" — not built); income_tax_slab bracket
// *restructuring* within one financial_year (this model can add/change a bracket but not remove
// one, since Shanti Ops treats a whole FY's slab set as one unit); effective_to is not factored
// into "what's in force on date X", only effective_from ordering is.
export async function diffAndDraft(incoming, { submitted_by } = {}) {
  ...
  for (const row of existingRows) addToGroup(row.category, row.payload, { id: row.id, effective_from: row.effective_from, approved_at: row.approved_at });
